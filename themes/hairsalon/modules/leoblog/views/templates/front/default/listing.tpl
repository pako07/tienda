...
<div id="blog-listing" class="blogs-container box">
 	{if isset($filter.type)}
	 	{if $filter.type=='tag'}
			<h1>{l s='Filter Blogs By Tag' mod='leoblog'} : <span>{$filter.tag}</span></h1>
		{elseif $filter.type=='author'}
			<h1>{l s='Filter Blogs By Blogger' mod='leoblog'} : <span>{$filter.employee->firstname} {$filter.employee->lastname}</span></h1>
		{/if}
	{else}
	<h1 class="title_blog">{l s='Lastest Blogs' mod='leoblog'}</h1>
	{/if}
