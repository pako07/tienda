...
<div class="box box-small clearfix">
	...
	<form id="submitReorder" action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
		...
		<input type="hidden" value="" name="submitReorder"/>
			...
			<a href="#" onclick="$(this).closest('form').submit(); return false;" class="button btn btn-outline button-medium pull-right"><span>{l s='Reorder'}</span></a>
	</form>
	{/if}
	<p class="dark">
...
<div class="info-order box">
	...
	{/if}
	{if $order->recyclable}
	<p><i class="fa fa-repeat"></i>&nbsp;{l s='You have given permission to receive your order in recycled packaging.'}</p>
	{/if}
	{if $order->gift}
...
{if $return_allowed}
	<div id="returnOrderMessage">
		<h3 class="page-subheading bottom-indent">{l s='Merchandise return'}</h3>
		...
		<p class="form-group">
			<button type="submit" name="submitReturnMerchandise" class="btn btn-outline button button-small btn-sm"><span>{l s='Make an RMA slip'}</span></button>
...
{if !$is_guest}
	{if count($messages)}
	<h3 class="page-subheading">{l s='Messages'}</h3>
	...
	<form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std" id="sendOrderMessage">
		<h3 class="page-subheading bottom-indent">{l s='Add a message'}</h3>
		...
		<div class="submit">
			...
			<button type="submit" name="submitMessage" class="button btn btn-outline button-medium"><span>{l s='Send'}</span></button>
		</div>
	</form>
{else}
