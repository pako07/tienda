				...
				<div class="comment row" itemprop="review" itemscope itemtype="https://schema.org/Review">
					<div class="comment_author col-sm-2">
						<div itemprop="itemReviewed" itemscope itemtype="https://schema.org/Restaurant">
							<span itemprop="name">{l s='Grade' mod='productcomments'}&nbsp;</span>
						</div>
						...
						<ul>
							...
							{if $is_logged}
								...
								<li>
									...
									<button class="usefulness_btn btn btn-outline button button-small btn-sm" data-is-usefull="1" data-id-product-comment="{$comment.id_product_comment}">
										<span>{l s='Yes' mod='productcomments'}</span>
									</button>
									<button class="usefulness_btn btn btn-outline button button-small btn-sm" data-is-usefull="0" data-id-product-comment="{$comment.id_product_comment}">
			...
			<div class="row">
				{if isset($product) && $product}
					{if isset($product->name)}
						...
						{assign var="productDes" value=$product->description_short}
					{else if isset($product.name)}
						{assign var="productName" value=$product.name}
						{assign var="productDes" value=$product.description_short}
					{else}
						{assign var="productName" value=""}
						{assign var="productDes" value=""}
					{/if}
					<div class="product clearfix  col-xs-12 col-sm-6">
						<img src="{$productcomment_cover_image}" height="{$mediumSize.height}" width="{$mediumSize.width}" alt="{$productName|escape:'html':'UTF-8'}" />
