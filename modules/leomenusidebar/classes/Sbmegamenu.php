...
class Sbmegamenu extends ObjectModel
{
    ...
    public function add($autodate = true, $null_values = false)
    {
        ...
        $sql = 'INSERT INTO `'._DB_PREFIX_.'sbmegamenu_shop` (`id_shop`, `id_sbmegamenu`)
    ...
    {
        $sql = 'SELECT cp.`id_sbmegamenu`, cp.`position`, cp.`id_parent`
...
//					$t = '%s';
        ...
        switch ($menu['type']) {
            ...
            case 'url':
                ...
                if (preg_match('/http:\/\//',$menu['url'])  ||  preg_match('/https:\/\//',$menu['url'])  ){
                    ...
                    $host_name = LeoSbmegamenuHelper::getBaseLink().LeoSbmegamenuHelper::getLangLink();
    ...
    public function getGroups($parram = null)
    {
        $groupBox = array();

        if($parram == null){
            // process of this object at backend
            if($this->groupBox && strtolower($this->groupBox) == 'all')
            {
                $groupBox[0] = 'all';
